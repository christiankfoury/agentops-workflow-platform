...
export function LiveRefresh({ scope, kind, id, terminal = false }: {
  ...
  const [refreshing, startRefresh] = useTransition(), refreshPending = useRef(false);
  useEffect(() => { refreshPending.current = refreshing; }, [refreshing]);
  useEffect(() => {
    let active = true;
    const loop = startPolling({
      read: () => readPulse(scope, kind, id),
      refresh: () => startRefresh(() => router.refresh()), status: text => queueMicrotask(() => { if (active) setStatus(text); }),
      available: () => navigator.onLine !== false && document.visibilityState !== "hidden" && !refreshPending.current,
  ...
  }, [scope, kind, id, terminal, router, startRefresh]);
