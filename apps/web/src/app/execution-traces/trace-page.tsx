...
export async function TracePageView({ id, legacy = false }: { id: string; legacy?: boolean }) {
  ...
  return <div className="space-y-5"><Link href="/execution-traces" className="underline">All graph runs</Link><TraceDebugger key={`${access.organization_id}:${id}`} head={head} initial={initial} initialError={initialError} scope={access.organization_id} read={readTrace} /></div>;
}
